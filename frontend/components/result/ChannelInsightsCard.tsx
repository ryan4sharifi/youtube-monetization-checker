...
export default function ChannelInsightsCard({
  ...
  return (
    <section
      className={`relative overflow-hidden rounded-[32px] border border-[var(--border)] bg-[color:color-mix(in_srgb,var(--card)_94%,transparent)] ${
        variant === "compact" ? "p-4 md:p-5" : "p-6 md:p-8"
      } shadow-[0_22px_70px_rgba(15,23,42,0.10)] backdrop-blur`}
    >
      ...
      <div className="relative z-10">
        ...
        <div
          className={`mt-4 grid gap-3 ${
            variant === "compact"
              ? "grid-cols-2"
              : "md:grid-cols-2 xl:grid-cols-4 md:gap-5"
          }`}
        >
          <div
            className={`rounded-[24px] border border-[var(--border)] bg-[color:color-mix(in_srgb,var(--background-elevated)_92%,transparent)] ${
          ...
          >
            ...
            <p className="mt-2 text-lg font-semibold capitalize tracking-[-0.03em] text-[var(--foreground)]">
              {prettify(safe.business_potential)}
            </p>
          </div>
        </div>
